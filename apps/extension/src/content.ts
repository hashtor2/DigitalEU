...
function findEmailInputFields(hints?: { selector?: string; textContentHint?: string; ariaLabelHint?: string }): HTMLInputElement[] {
  const inputs = Array.from(document.querySelectorAll("input"));
  
  // 1. Explicit Selector
  if (hints?.selector) {
    const matched = Array.from(document.querySelectorAll(hints.selector)) as HTMLInputElement[];
  ...
  return inputs.filter((input) => {
    ...
    if (hints?.ariaLabelHint) {
      ...
      if (ariaLabel.includes(hint)) return true;
      if (ariaLabelledBy) {
        const labelEl = document.getElementById(ariaLabelledBy);
        if (labelEl && labelEl.textContent?.toLowerCase().includes(hint)) return true;
      }
    }
    ...
    if (hints?.textContentHint && id) {
      ...
      if (labels.some(l => l.textContent?.toLowerCase().includes(hint))) return true;
    }
...
function injectAutofillButtons(hints?: { selector?: string; textContentHint?: string; ariaLabelHint?: string }) {
