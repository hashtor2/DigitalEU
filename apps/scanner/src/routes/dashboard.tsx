  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-mono font-bold">Your dashboard</h1>
          <p className="text-[#1a2332]/70 mt-1">Signed in as {user?.email}</p>
        </div>
        <button
          onClick={() => supabase.auth.signOut().then(() => (window.location.href = '/'))}
          className="px-4 py-2 rounded border border-[#1a2332]/20 font-mono font-semibold hover:bg-[#1a2332]/5 transition"
        >
          Sign out
        </button>
      </div>
      ...
      {scans.length === 0 ? (
        <div className="rounded-lg border border-[#1a2332]/10 bg-white p-6 text-center">
          <p className="text-[#1a2332]/70">No scans yet. Connect your email to get started.</p>
        </div>
      ) : (
        <div className="space-y-4">
          <h2 className="text-xl font-mono font-semibold">Recent scans</h2>
          {scans.map((scan) => (
            <div key={scan.id} className="rounded-lg border border-[#1a2332]/10 bg-white p-4">
              <div className="flex items-center justify-between">
                <div>
                ...
                <a href={`/results/${scan.id}`} className="text-[#c17a5c] font-mono font-semibold hover:underline">
                  View results →
                </a>
              </div>
            </div>
